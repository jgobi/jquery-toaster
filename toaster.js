$.toaster = {
		...
		setProps: function(options){
			...
			var willCallFixPositionFunction = false;
			if (typeof options.initialTop == 'number')
				willCallFixPositionFunction = true;
				
			if (typeof options.appearOnLeft == 'boolean'){
				$.toaster.appearOnLeft = options.appearOnLeft;
				willCallFixPositionFunction = true;
			}
			...
			if(willCallFixPositionFunction){
				$.toaster.fixPosition((typeof options.initialTop == 'number') ? options.initialTop : $.toaster.initialTop);
			}
		},
		
		fixPosition: function(newInitialTop){
			/**
		...
		show: function(type,message,options){
			...
			if( $(window).width() <= 720 ){ //smartphones - will show the toast in the bottom, and only one once.
				if((typeof options.closeOnClick == 'boolean' && options.closeOnClick) || (typeof options.closeOnClick == 'undefined' && $.toaster.closeOnClick)) //closeOnClick
					var toast='<div onclick="$.toaster.hide('+r+')" ';
				else //close only on X
					var toast='<div ';
					
				toast+='style="z-index: 999999; width: 100%; position: fixed; left: 0; bottom: 0px; margin:0px" id="toast-' +r+'" data-alert class="alert-box '+type+'">'+message+' &nbsp;&nbsp;&nbsp;<a href="#" onclick="$.toaster.hide('+r+')" class="close">&times;</a></div>';
